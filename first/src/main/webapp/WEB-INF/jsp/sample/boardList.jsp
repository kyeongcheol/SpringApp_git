...
<%@ include file="/WEB-INF/include/include-body.jspf" %>
 ...
 $(document).ready(function()
 {
	 ...
	 $("#write").on("click", function(e) //쓰기 페이지
	 {
		 e.preventDefault(); //이벤트 전파 방지
		 fn_openBoardWrite(); //메서드 호출
	 });
	 
	 //id가 a태그의 name이 title인 것을 클릭하면..
	 $("a[name='title']").on("click", function(e) //상세보기 페이지
	 {
		 e.preventDefault(); //이벤트 전파 방지
		 //this : jQuery 객체 의미, a 태그의 name이 title
		 fn_openBoardDetail($(this)); //메서드 호출
	 });
 });
 ...
 //페이징 콜백 함수
 function fn_selectBoardCallback(data)
 {
	...
	else
	{
		...
		gfn_renderPaging(params); //common.js에 페이징 로직 처리 함수 호출
