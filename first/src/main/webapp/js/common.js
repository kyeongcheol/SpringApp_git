...
function ComSubmit(opt_formId) 
{
	...
	this.addParam = function addParam(key, value)
	{
		...
		$("#"+this.formId).  
		append($("<input type='hidden' name='"+key+"' id='"+key+"' value='"+value+"' >")); 	//받아온 key와 value를 input 태그에 mapping
...
function ComAjax(opt_formId)
{
   ...
   this.url = "";
   this.formId = gfn.isNull(opt_formId) == true ? "commonForm" : opt_formId;
   this.param = "";
...
pageIndex : 현재 페이지(currentPage) 위치가 저장될 input 태그 id
recordCount : 페이지 당 레코드 수
totalCount : 전체 조회 건수
...
function gfn_renderPaging(params)
{
    ...
    {
    	recordCount = 20; //페이지당 게시물 수 지정
    }
	
    // ex) 500 / 20 = 25 => totalIndexCount 
    var totalIndexCount = Math.ceil(totalCount / recordCount); //전체 인덱스 수
    gfv_eventName = params.eventName; // 콜백함수 지정
